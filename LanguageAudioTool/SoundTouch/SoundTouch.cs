...
namespace VarispeedDemo.SoundTouch
{
    class SoundTouch : IDisposable
    {
        ...
        public void SetSequenceMS(int sequenceMS)
        {
            if (is64Bit)
                SoundTouchInterop64.soundtouch_setSetting(handle, SoundTouchSettings.SequenceMs, sequenceMS);
            else
                SoundTouchInterop32.soundtouch_setSetting(handle, SoundTouchSettings.SequenceMs, sequenceMS);
        }
